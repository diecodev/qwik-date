...
export function generateInlineMonthsObject({
  ...
  if (actualYear === maxDateYear && actualMonth === maxDateMonth)
    nextDate = maxDate;

  if (actualYear === minDateYear && actualMonth === minDateMonth)
    prevDate = minDate;
...
const getPrevMonthDate = (
  ...
  const prevMonth =
    activeDate.getMonth() === 0 ? 11 : activeDate.getMonth() - 1;
  const prevYear =
...
const getNextMonthDate = (activeDate: Date, nextMonthDay: number) => {
  const nextMonth =
    activeDate.getMonth() === 11 ? 0 : activeDate.getMonth() + 1;
  const nextYear =
